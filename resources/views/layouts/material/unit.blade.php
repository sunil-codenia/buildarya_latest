                ...
                @if (checkmodulepermission(3, 'can_view') == 1)
                    <div class="body">
                        <div class="table-responsive">
                            <form id="bulkActionForm" action="{{ url('/materialunit/bulk_action') }}" method="POST">
                                @csrf
                                <input type="hidden" name="bulk_action" id="bulk_action_input">
                                <table id="materialUnitTable" class="table table-hover">
                                    <thead>
                                        <tr>
    ...
    <script type="text/javascript">
        ...
        function bulkEdit() {
            if (selectedUnitIds.size > 0) {
                syncUnitFormInputs();
                $('#bulkActionForm').attr('action', "{{ url('/bulk_edit_unit') }}");
                $('#bulkActionForm').submit();
            } else {
                Swal.fire({
                    title: 'No Items Selected',
                    text: 'Please select at least one Material Unit to edit.',
                    icon: 'info',
        ...
        function submitBulkAction(action) {
            if (selectedUnitIds.size === 0) {
                Swal.fire({
                    title: 'No Items Selected',
                    text: 'Please select at least one Material Unit.',
                    icon: 'info',
                    confirmButtonColor: '#343a40'
                });
                return;
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "You are about to " + action + " all selected material units!",
                icon: 'warning',
                showCancelButton: true,
                toast: true,
                position: 'center',
        ...
        $(document).ready(function() {
            ...
            function updateSelectAllUnitState() {
            ...
            $("#select_all").click(function() {
                ...
                $('.check_item').each(function() {
                    ...
                    if (isChecked) {
                        selectedUnitIds.add(this.value);
                    } else {
                        selectedUnitIds.delete(this.value);
                    }
                });
                updateToolbar();
            });
            ...
            $(document).on('change click', '.check_item', function() {
                ...
                updateSelectAllUnitState();
                updateToolbar();
            });

            $('#materialUnitTable').DataTable({
                serverSide: true,
                processing: true,
                ajax: {
                    url: "{{ url('/materialunit_ajax') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}"
                    }
                ...
                drawCallback: function(settings) {
                    $('.check_item').each(function() {
                        if (selectedUnitIds.has(this.value)) {
                            this.checked = true;
                        } else {
                            this.checked = false;
                        }
